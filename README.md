<div align="center">
<img src="assets/audica-wordmark.png" alt="audica" width="240">

### Turn the numbers into the human story.

</div>

---

A marketing mix model MMM gives you one number per channel and a way to split the budget. That number is a mean, the center of a distribution, and statistics is clear about one thing: a mean without its spread is only half the answer.

<div align="center">
<img src="assets/audica-sand.png" alt="The model draws the curve. Audica reads the sand." width="640">

<em>The model draws the curve. Audica reads the sand.</em>

</div>

When the model says a channel drove twenty percent, that same twenty percent can be consistent with anything from ten to forty, depending on how wide the distribution is. The tool reports the range as an interval and hands you the center. And when the spread is that wide, splitting the budget on the mean alone is closer to a coin toss than to a decision.

What you actually need is in the shape, not the center. The width tells you how far to trust the number. The grains far from the mean, the tails, are not noise. They are the customers, the moments and the contexts where a channel behaves nothing like its average: quietly excellent for some, quietly failing for others.

Audica reads the shape. It turns the distributions your model already produces into a reading of your audience: how far to trust each number, which channels behave the same for everyone and which hide very different people, and where the budget should truly go.

MMM tell you what happened, on average. Audica tells you what it means, and for whom.

Open source, Apache 2.0. Research presented in the Third Workshop on Social Influence in Conversations (2025. Built by Javier Marín.
